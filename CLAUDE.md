...
### `biblioteca_gif`
**1.606 righe** (23 agosto 2026): 624 vive, 0 rotte, 63 libere, **919 morte** (cantiere 3E). Le libere passano da 43 a 63 per due movimenti opposti: **+21 righe doppie di Spalle e Cuffia**, inserite il 23 agosto col nome nuovo e lo stesso `storage_path` — risolvono già, e restano libere finché il secondo sync non porta i `gif_slug` nuovi a catalogo — e **−1**, `alzate-laterali-fianco-manubrio-panca-inclinata-45-gradi`, che è passata a **morta**: era la riga di EX408, e il file a cui puntava è stato rinominato dal passo 1. Si cancella con le altre righe vecchie, dopo il secondo sync. La riga libera in più è `alzate-laterali-fianco-manubrio-panca-inclinata-45-gradi`, rimasta senza codice quando EX408 è stato consolidato in EX057: punta lo stesso file della riga di EX057 e si cancella alla fine dell'ordine a righe doppie, non prima. Le 4 righe morte che avevano `storage_path` in `Gambe e Glutei` — `affondo-bulgaro-corpo-libero`, `leg-extension`, `ponte-dei-glutei`, `squat`, tutte del 28 giugno-2 luglio, nessun codice e nessun file — sono state cancellate il 22 agosto sera. Backup in `tools/biblioteca-nomi/_backup/`. Le 918 rimaste hanno tutte `storage_path` in una cartella legacy (`muscolazione` 665 · `stretching` 135 · `calisthenics` 118) e **nessuna è puntata da un codice**. Conteggi sempre aggiornati in [`docs/STATO.md`](docs/STATO.md). Colonne: `slug, nome_italiano, nome_originale, categoria, gruppo_muscolare, storage_path, storage_url`. `slug` = `gif_slug` del catalogo.
...
⚠️ **La replica non sta su disco: si riscrive dal sorgente a ogni misura, e questo la rende un termine di paragone incerto.** Il 22 agosto sera è stata ricostruita leggendo `_trainGenFilterPool` riga per riga. Sul catalogo simulato *prima* del sync restituisce `poolPrincipali` **376** e `poolCarry` **1** — identici alla baseline documentata — ma `poolFinisher` **214** contro 208, `poolRiscaldamento` **44** contro 43 e ammesse **517** contro 516. **Quei tre scarti non vengono dal sync**: vengono da una differenza fra questa replica e quella che aveva misurato i numeri precedenti, che nessuno ha conservato. Finché la baseline non si legge da `?schedaDebug=1` in app, uno scarto di poche unità sui pool secondari non prova niente. Lo strumento andrebbe scritto una volta e tenuto in `tools/`.
