...
- **Audit diagnosi completata**: mappa problemi 4 rossi / 6 gialli / 8 note (dettaglio in `mappa-audit-training.md` di Ignazio). Punti ancora aperti: 91 esercizi senza `gif_slug` (di cui 53 principali) · sessioni fallback hardcoded senza codice · rientro soft dopo pause lunghe assente · infortunio solo giornaliero · `GEAR_ALIASES` corto · 1.164 GIF orfane in biblioteca. ~~Slug rotti EX057/EX088~~ chiusi il 17 luglio sera (fix `gif_slug` via Sheet + sync, riuso GIF esistenti: nessun upload).
