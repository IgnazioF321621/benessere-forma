...
**Metodo di lavoro consolidato (da ripetere)**: ispezione DB/catalogo PRIMA di assumere (Lezione 1); fasi separate con `AskUserQuestion` ai bivi; `node --check` sullo script inline dopo ogni fase; harness Node (`new Function(genBlock)` + mock `supa`/`ST`) per provare la pipeline senza browser; `git diff` completo + verifica "nessuna riga rimossa non voluta" prima del commit; commit singolo solo su OK esplicito.

#### Ispezione preliminare (codici fusi, pre-sync)
Prima del sync il catalogo aveva 9 doppioni-attrezzo che sono stati FUSI (EX004/005/007/010/012/014/020/025/056 → assorbiti nelle madri EX002/006/009/011/013/018/024/055). Ispezione: **nessuno di quei 9 codici è hardcoded** in `zona-tracker.html` (il generatore non usa `codice` per pescare — lavora su pattern/gruppo_target/attrezzo). Le 15 schede storiche in `schede_utente` (tutte di Ignazio) referenziavano EX005/012/014/025/056 ma sono **name-based e self-contained** → nessun riferimento rotto in lettura (`loadActiveScheda` non ri-interroga il catalogo). I codici orfani diventano irrilevanti alla prossima rigenerazione. Nessuna migrazione necessaria.

#### FASE 1 — Cuffia rotatori (warm-up) + nuovi isolamenti bonus + **FIX filtro attrezzo multiplo**
- **FIX CRITICO scoperto in ispezione (non nel brief)**: il filtro attrezzo "casa" in `_trainGenFilterPool` confrontava la STRINGA INTERA `attrezzo` (normalizzata) col set utente. Col catalogo nuovo **38/123 esercizi hanno `attrezzo` multiplo** (es. `'elastico;manubri'`) → venivano TUTTI esclusi a casa anche possedendo uno degli attrezzi. Effetto sul profilo Ignazio: pool **57→96 esercizi**. Fix: split su `;`, entra se possiede **almeno uno** degli attrezzi nativi (`attrezziNativi.some(...)`). Stesso fix gemello sul ramo `'aperto'` (whitelist portatili). Per attrezzo singolo comportamento identico a prima → retrocompatibile.
- **Cuffia dei rotatori = PREHAB nel warm-up Upper** (mai isolamento/bonus): pescata da `uso='riscaldamento'` per `gruppo_target='cuffia rotatori'` (EX101), inserita in un **campo SEPARATO `warmup`** sulla sessione (come `finisher`). Carico leggero 2×15-20, mai a cedimento.
- **Nuovi isolamenti come BONUS, mai obbligatori** (già coperti dai compound): `_TRAIN_GEN_BONUS_BY_MACRO` esteso → lower `+quadricipiti`, upper `+petto/dorsali/deltoidi anteriori`.
- Nuovi helper: `_trainGenMapWarmupExercise`, `_trainGenPickWarmup` (per gruppo_target) + nuovo pool `poolRiscaldamento` in `_trainGenFilterPool`.
- ⚠️ **Limite di DATO** (catalogo, non codice): `EX099 Pull-over` ed `EX100 Alzate frontali` hanno `uso='finisher'` (non `principale`) → non entrano come bonus iso finché il Sheet non li marca anche `principale`. EX097 Leg extension / EX098 Croci entrano (`principale;finisher`).

#### FASE 2 — Loaded carry come `carry_conclusivo` + rinomina label Tabata
- **Chiarimento terminologico (deciso dall'utente)**: "finisher" era ambiguo. La **scritta visibile** del blocco Tabata è stata cambiata da "FINISHER TABATA" → **"TABATA FINALE"** (1 sola occorrenza UI in `renderTraining`; il **campo dati `finisher` nel codice resta invariato** — rinominarlo è rischio inutile).
- **Nuovo campo SEPARATO `carry_conclusivo`** (distinto dal Tabata, come `warmup`): pescato da `pattern='loaded carry'` + tag `finisher` → entrano **solo farmer walk (EX088) + suitcase carry (EX089)**. Overhead walk / turkish get-up hanno `uso='carry'` senza `finisher` → esclusi (get-up troppo tecnico, overhead per cautele). Guard extra anti-drift sul nome per i get-up.
- **1-2 carry a settimana, NON in ogni sessione**, distribuiti su sessioni equispaziate, a rotazione farmer/suitcase (`carryTotal = 1 se ≤3 giorni, 2 se ≥4`). Nuovo pool `poolCarry` + helper `_trainGenMapCarryExercise` (gestisce surrogato casa: `nota_surrogato` sostituisce setup, `eq`=attrezzo surrogato, reps `20-30 sec per lato` isometrico da fermo).

#### FASE 3 (scope ristretto) — Warm-up Lower
- **Nodo scoperto**: G3/G6 ("Recupero Mobilità"/"Stretching") NON sono gusci vuoti come diceva il brief — sono **hardcoded in `TRAINING_SESSIONS`** (recoveryUpper riga ~7302, recoveryLower ~7427) con ~23-28 micro-esercizi del 12-13 mag, fuori dal generatore e dalla rotazione 6-giorni hardcoded (`SESSION_CYCLE`/`DAY_SPLIT`). **Decisione utente: G3/G6 RIMANDATI** (riempirli dal catalogo è intervento a sé). **Rendering in app RIMANDATO a fase separata.**
- Quindi FASE 3 = solo **dati warm-up Lower** nel generatore: mobilità anca/glutei (EX117/119/120) nel campo `warmup`. Questi esercizi hanno `gruppo_target` VUOTO → nuovo helper `_trainGenPickWarmupByMuscle` che pesca per **muscoli** (substring) ristretto a `pattern='mobilita'`. Fino a 2 esercizi, round-robin (Lower A ≠ Lower B).

#### FASE 4 — Preferenza sicurezza nel picker compound (zone_rischio)
- `_trainGenPickByPattern` esteso con parametro opzionale `limitazioni`. Quando passato (SOLO dalla chiamata **compound**, non dai bonus), a parità di pattern **preferisce le varianti senza `zone_rischio` in conflitto** col profilo, e fa round-robin solo tra quelle sicure.
- **È una PREFERENZA, non un filtro**: se tutte le varianti sono rischiose → fallback all'intero set + rete `_trainGenApplyCautions` (adatta→sostituisci→skip). Nessun pattern resta mai scoperto. Riusa il `zone_rischio` del catalogo, nessun hardcoding di codici.
- **Verificato (harness, profilo lombare+ginocchia)**: Upper evita EX001/EX006/EX011 (rischiosi) → sceglie varianti pulite. **Lower**: tutti i compound dominante-ginocchia/anca hanno lombare o ginocchia nel `zone_rischio` → fallback a cautele, Squat/Stacco rumeno **tenuti con `adattamento`** (corretto da preparatore: per le gambe si adatta, non si elimina, altrimenti il Lower resta senza pattern dominanti). Box jump/Broad jump sono `cardio_metabolico` → solo pool Tabata.

#### Bug di collaudo risolto (era OSSERVAZIONE, non DATO)
Durante il collaudo locale `warmup`/`carry_conclusivo` "sparivano" nel raw di `ztSchedaWhy`. **Diagnosi con harness Node**: i campi erano GIÀ nell'oggetto sessione e nella scheda salvata (sopravvivono a `_trainGenValidateCodes` e allo strip `_diag` perché sono campi separati come `finisher`). Il problema era nelle **proiezioni diagnostiche** che li omettevano: la `.map()` "dettaglio esercizi (raw)" di `ztSchedaWhy` e il log `[train-gen] sessioni finali (mapped)` elencavano solo alcuni campi. Fix: aggiunti `warmup`/`carry_conclusivo` a entrambe le proiezioni. **Lezione: "mostra A invece di B" può essere un problema di VISTA, non di dato — l'harness fuori dal browser lo dimostra in 30 secondi.**

#### Carry EX088/EX089 — problema di DATO (non di codice), fix a Ignazio nel Sheet
...
- **Lista unilaterali** — costante `_TRAIN_GEN_UNILATERAL` (vicino a `_TRAIN_GEN_SOFT_MAX`), **11 esercizi**: `['EX015','EX033','EX035','EX038','EX039','EX041','EX047','EX066','EX062','EX067','EX068']` = Affondi · Step-up al ritmo · Single-leg glute bridge · Affondo camminato · Affondo posteriore · Step-up alto su panca · Bulgarian split squat · Calf raise unilaterale · **EX062 Tricipiti kickback con manubri** · **EX067 Abduzione laterale anca con elastico** · **EX068 Kickback gluteo con elastico** (questi 3 aggiunti nel giro successivo, erano sfuggiti).
...
- ~~**Disallineamento slug attrezzatura:** l'onboarding nuovo salva `elastici_tubo`, il profilo di Ignazio ha `elastico`. Riguarda solo utenti futuri — da uniformare (slug onboarding vs valori catalogo).~~ ✅ RISOLTO 31 mag 2026 (commit `fec026d`, APP_VERSION `2026.05.31 · 21:25`). Rubrica alias **Modo A** in `_trainGenFilterPool` (subito dopo `attrezzaturaSet.add('corpo_libero')`): mappa `GEAR_ALIASES = { elastici_tubo→elastico, cavigliere→cavigliera, barra_corta→barra, barra_lunga→barra }`, aggiunge il termine-catalogo al set SENZA rimuovere lo slug originale → retrocompatibile coi profili vecchi (`elastico` già giusto = no-op, provato byte-identico). NON tocca DB profili né onboarding. Collaudo Node harness 12/12. ⚠️ Agisce alla **generazione** → i tester con scheda già salvata devono rigenerarla per vederne l'effetto.
