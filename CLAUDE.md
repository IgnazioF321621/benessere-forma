...
**Rename + 2 nuovi esercizi zona Gambe e Glutei, 8 luglio 2026** — allineamento `nome`/`nome_en` a convenzione semplificata (drop "bilaterale/simultaneo") su 109 codici esistenti (EX013–EX293, esclusi EX222/EX271 riservati a Bicipiti). Storage rinominato per i 92/109 codici con `gif_slug` (bucket `Gambe e Glutei/`, ° stripped nei filename), `biblioteca_gif` aggiornato di conseguenza — verificati tutti raggiungibili (HEAD 200). `training_logs`: 9 righe migrate (4× EX081 "Squat frontale...", 5× EX254 "Ponte glutei frog pump..."). **Fusione EX207**: eliminato "Affondo alternato camminato con manubri" (asset fornitore byte-identico a EX212 "pliometrico con manubri") — 0 riferimenti `alternativa`/`training_logs` verificati prima della cancellazione. **2 nuovi codici**: EX322 "Affondo indietro manubri" (gruppo `quadricipiti`), EX323 "Affondo corpo libero" (gruppo `quadricipiti`) — nessun `gif_slug` per ora. **EX324 "Camminata manubri" scartato su conferma utente**: duplicava EX088 "Farmer walk" già in catalogo (stesso pattern `loaded carry`, stessi muscoli, stesso attrezzo). ⚠️ Il TSV sorgente usava `pattern`/`gruppo_target` fuori dal vocabolario chiuso (`affondo`/`camminata carico`/`gambe`) per i 3 nuovi codici — normalizzati a `composto`/`quadricipiti` (EX322/EX323) su conferma utente, coerenti con gli esercizi affini già in catalogo. **Segnalati, non corretti** (richiesta esplicita, sessione dedicata GIF): EX211 "Affondo laterale manubri" (GIF non pertinente) ed EX291 "Stacco rumeno una gamba manubri" (nessuna GIF valida) — `gif_slug` invariato per entrambi.

**EX088 aggiornato + collegamento GIF, 8 luglio 2026** — `nome` "Farmer walk"→"Camminata manubri", `nome_en`="Dumbbell farmer's walk", `gruppo_target`="quadricipiti". Asset locale (`Camminata alternata con manubri (Dumbbell farmer's walk).gif`) risultava già su Storage sotto `Gambe e Glutei/Camminata alternata con manubri.gif` (slug `camminata-alternata-manubri`, 0 riferimenti in `esercizi_catalog` prima di questa modifica) — riutilizzata la riga esistente: file rinominato su Storage a `Camminata manubri (Dumbbell farmer's walk).gif`, `biblioteca_gif.nome_italiano`/`storage_path`/`storage_url` aggiornati (slug invariato), EX088.gif_slug collegato. Eliminato un duplicato orfano trovato per hash-match durante la verifica (`Gambe e Glutei/Farmer's walk con manubri.gif`, stesso contenuto, 0 riferimenti). ⚠️ **EX323 "Affondo corpo libero" NON collegato**: l'asset locale richiesto è byte-identico a un file già su Storage e già assegnato a EX015 "Affondi" (slug `affondi-corpo-libero`) — le due voci hanno `esecuzione` diverse (EX015 descrive un affondo con passo avanti, EX323 un affondo sul posto), possibile incoerenza pre-esistente su EX015 o doppione di EX323, segnalato all'utente e lasciato in sospeso senza decisione (nessuna modifica a EX323, `gif_slug` resta NULL).

**Fix EX015/EX323, 8 luglio 2026** — risolto il conflitto sopra su conferma utente (duplicazione intenzionale). EX015 rinominato "Affondi"→"Affondo alternato corpo libero" (`biblioteca_gif.nome_italiano` aggiornato di conseguenza, slug/asset `affondi-corpo-libero` invariati). EX323 rinominato "Affondo corpo libero"→"Affondo unilaterale corpo libero", collegato a **nuova riga** `biblioteca_gif` (slug `affondo-unilaterale-corpo-libero`) che punta allo stesso file Storage di EX015 (`Gambe e Glutei/Affondo unilaterale sul posto a corpo libero.gif`) — 2 codici, 2 slug, 1 solo asset fisico condiviso by design. ⚠️ **Segnalazione aperta**: il nuovo nome di EX015 ("alternato" = passo avanti alternato) non corrisponde al contenuto reale dell'asset (`nome_originale` = "Stationary bodyweight lunge", affondo fermo sul posto) — mismatch semantico invariato dal batch precedente, potrebbe servire un asset nuovo per EX015 in una sessione dedicata GIF.
