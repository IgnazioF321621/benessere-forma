#!/usr/bin/env python3
# Patch: dose_unit (tendina) + dose_multiplier (frazione dose)
# Esegui con: cd ~/benessere-forma && python3 patch_doseUnit.py
...
# ── PATCH 2: renderIntegratori — sostituisci doseUnit/doseUnitLabel con select ─
...
NEW2 = """      // Unità dal catalogo come default, sovrascrivibile dall'utente
      const doseUnitDefault = (() => {
        const n = (s.name||'').toLowerCase();
...
# ── PATCH 3: macro-grid — sostituisci cella Dose/die con select unità + multiplier
...
# ── PATCH 4: nuova funzione updateSuppMultiplier dopo updateSuppDose ─────────
OLD4 = """// ── CATALOG MODAL ─────────────────────────────────────────
...
# ── PATCH 5: suppMonthlyCost — applica dose_multiplier al costo ──────────────
...
if errors:
    ...
    print("git add -A && git commit -m 'feat: dose_unit tendina + dose_multiplier' && git push origin main")
