...
**La quarta comparsa, e chi l'ha trovata** *(16 agosto, poche ore dopo)*. Rinominare i campi non aveva chiuso tutto: `pianifica.py` agganciava il file del Mac al suo oggetto confrontando lo SHA-256 del Mac con le impronte del bucket, che dal 15 agosto contiene i byte ridotti. Misurato a migrazione di Pettorali conclusa: **0 impronte del Mac su 82 presenti nel bucket, 82 su 82 quelle ridotte**. Il piano rigenerato dichiarava tutte e 82 le righe `nuova`, con 82 collisioni di slug — cioè «ricarica da zero una zona che è già a posto».

A trovarla non è stato un sospetto, ma il controllo introdotto dalla [decisione sui piani](CANTIERI.md#-decisione-del-16-agosto-2026--i-piani-si-rigenerano-a-fine-migrazione): *a fine migrazione il piano si rigenera e deve dare tutto `slug invariato` con `percorso che cambia: 0`*. Un atteso dichiarato in anticipo trasforma una rigenerazione di routine in una verifica, e questa ha parlato al primo colpo. **Un controllo vale quanto l'atteso che gli si scrive accanto**: senza quel numero, un piano con 82 righe `nuova` sarebbe passato per il risultato normale di uno strumento appena rilanciato.
...
**La quinta comparsa, e perché la quarta non l'aveva chiusa** *(23 agosto)*. Riparare `pianifica.py` aveva tolto il difetto da `pianifica.py`. Non dagli altri. `prepara.py` — che sta **a monte**, ed è il primo passo del lavoro 1 di ogni zona — agganciava ancora `paths = per_sha_bucket.get(sha, [])` e basta. Misurato su Tricipiti prima di toccarlo: **0 impronte del Mac su 61 presenti fra quelle del bucket**, e la classificazione usciva `collegato 0 · pendente 5 · indicizzato 0 · libero 57`, contro i 55/5/0/2 veri. Il pannello si sarebbe aperto dichiarando **libere 57 GIF di cui 54 puntate da un codice vivo**, e — peggio del numero — avrebbe proposto i nomi dedotti dal nome del file, che lo strumento stesso definisce «la fonte più debole», invece che dal catalogo. Ogni conferma data lì finisce sul registro append-only nell'istante in cui è data [L21]: sarebbe stato lavoro manuale sbagliato e già salvato.

**La causa non era il codice mancante, era il codice duplicato.** La quarta riparazione è stata scritta dentro `pianifica.py` come funzione locale, e in quel momento `prepara.py` conteneva la stessa logica di aggancio scritta a mano. Due esemplari della stessa decisione: riparare l'uno non tocca l'altro, e nulla segnala che l'altro esista. Per questo la quinta riparazione **non è una copia**: `prepara.py` fa `from pianifica import ponte_480`. Un esemplare solo, e la sesta comparsa non può più nascere da una divergenza fra copie — solo da un terzo chiamante che non lo importa affatto, che è un errore molto più visibile.

**Il numero non conta più, conta la classe.** L35 dice «alla terza volta si corregge il nome». I nomi erano già stati corretti alla terza, e alla quarta e alla quinta il difetto è tornato lo stesso, perché la condizione non era più il nome: era **l'esistenza di due artefatti per lo stesso esercizio**, e ogni punto del codice che ne aggancia uno all'altro deve passare per il piano dei 480px. Quel passaggio ora ha un nome proprio, `ponte_480`, e vive in un posto solo. Il criterio generale: **quando la riparazione è una funzione, il rimedio non è scriverla bene una volta — è fare in modo che non se ne possa scrivere una seconda.**
...
## L48 — Quando si corregge la fonte di un numero, si cercano tutti i posti che rispondono alla stessa domanda

**Il caso.** Il 12 settembre il quadro settimanale aveva scoperto che le pesate di Ignazio stanno in `weight_logs` e non in `body_logs` ([L47](#l47--lo-schema-dice-dove-un-dato-può-stare-le-righe-dicono-dove-sta)), e aveva scelto la regola giusta: una pesata al giorno, `weight_logs` > `body_logs` > misure del check. La regola però era stata scritta **dentro il quadro**, per il quadro.

Il 13 settembre il brief della card peso chiedeva «i numeri coincidono con il tab Body». Non coincidevano: il numero grande del tab Body, la pillola del peso in alto e la card Body in Home passavano da `getLatestBodyData()`, che `weight_logs` non l'ha mai letta. Per Ignazio dicevano **69,95** — il check del 2 agosto, sei settimane prima — mentre la pesata di quella mattina era **72,1**. Il difetto c'era da maggio, da quando esiste il foglio «Pesati ora»; il giorno prima era stato trovato, e lasciato dov'era in tre schermate su quattro.
